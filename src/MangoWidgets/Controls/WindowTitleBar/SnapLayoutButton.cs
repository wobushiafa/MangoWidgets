...
internal class SnapLayoutButton
{
    ...
    /// </summary>
    public SnapLayoutButton(MangoWidgets.Controls.Button button, TitleBarButton type, double dpiScale)
    {
    ...
    public bool IsMouseOver(IntPtr positionPointer)
    {
        ...
        // Invalid button size
        if (_renderedSize is { Height: 0, Width: 0 })
            return false;

        var positionPointerLong = positionPointer.ToInt64();

        if (positionPointerLong is > 2147483647 or < -2145629296)
            return false;
